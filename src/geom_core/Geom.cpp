...
void Geom::CreateDegenGeom( VspSurf &surf, int isurf, DegenGeom &degenGeom, bool preview, const int & n_ref )
{
    ...
    //==== Tesselate Surface ====//
    UpdateTesselate( surf, m_CapUMinSuccess[ m_MainSurfIndxVec[isurf] ], m_CapUMaxSuccess[ m_MainSurfIndxVec[isurf] ], true, pnts, nrms, uwpnts, n_ref );
    surf.ResetUSkip();
...
vector< TMesh* > Geom::CreateTMeshVec( const vector<VspSurf> &surf_vec, bool skipnegflipnormal, const int & n_ref ) const
{
    ...
    for ( int i = 0 ; i < nsurf ; i++ )
    {
        ...
        {
            UpdateTesselate( surf_vec[i], m_CapUMinSuccess[ m_MainSurfIndxVec[i] ], m_CapUMaxSuccess[ m_MainSurfIndxVec[i] ], false, pnts, norms, uw_pnts, n_ref );
