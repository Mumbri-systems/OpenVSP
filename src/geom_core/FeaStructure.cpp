...
void FeaPart::UpdateDrawObjs()
{
    ...
    for ( unsigned int j = 0; j < m_FeaPartSurfVec.size(); j++ )
    {
        ...
        int nu = m_FeaPartSurfVec[j].GetNumSectU();

        for ( int iu = 0; iu < nu; iu++ )
        {
            double u0 = (double) iu / nu;
            double u1 = (double) ( iu + 1 ) / nu;
            vec3d p00 = m_FeaPartSurfVec[j].CompPnt01( u0, 0 );
            vec3d p10 = m_FeaPartSurfVec[j].CompPnt01( u1, 0 );
            vec3d p11 = m_FeaPartSurfVec[j].CompPnt01( u1, 1 );
            vec3d p01 = m_FeaPartSurfVec[j].CompPnt01( u0, 1 );
        ...
        for ( int iu = nu; iu > 0; iu-- )
        {
            double u0 = (double) iu / nu;
            double u1 = (double) ( iu - 1 ) / nu;
            vec3d p0 = m_FeaPartSurfVec[j].CompPnt01( u0, 1 );
            vec3d p1 = m_FeaPartSurfVec[j].CompPnt01( u1, 1 );

            m_FeaHighlightDO[j].m_PntVec.push_back( p0 );
            m_FeaHighlightDO[j].m_PntVec.push_back( p1 );
        }
...
VspSurf FeaSlice::ComputeSliceSurf()
{
    ...
    if ( veh )
    {
        ...
        if ( m_OrientationPlane() == vsp::SPINE_NORMAL )
        {
            ...
            vec3d y_prime = max_length * y_axis * cos( M_PI / 4 ) + max_length * z_axis * sin( M_PI / 4 );
            vec3d z_prime = max_length * -1 * y_axis * sin( M_PI / 4 ) + max_length * z_axis * cos( M_PI / 4 );
        ...
        else
        {
            ...
            if ( m_OrientationPlane() == vsp::YZ_BODY || m_OrientationPlane() == vsp::YZ_ABS )
            {
                ...
                if ( m_YRot() > 0 )
                {
                    if ( std::abs( ( M_PI / 180.0 ) * m_YRot() ) > atan( ( del_x + 2 * x_off ) / ( del_z - 2 * z_off ) ) )
                    {
                        del_z_plus += std::abs( ( del_x + 2 * x_off ) / sin( ( M_PI / 180.0 ) * m_YRot() ) );
                    }
                    else
                    {
                        del_z_plus += std::abs( ( del_z - 2 * z_off ) / cos( ( M_PI / 180.0 ) * m_YRot() ) );
                    }

                    if ( std::abs( ( M_PI / 180.0 ) * m_YRot() ) > atan( ( del_x - 2 * x_off ) / ( del_z + 2 * z_off ) ) )
                    {
                        del_z_minus += std::abs( ( del_x - 2 * x_off ) / sin( ( M_PI / 180.0 ) * m_YRot() ) );
                    }
                    else
                    {
                        del_z_minus += std::abs( ( del_z + 2 * z_off ) / cos( ( M_PI / 180.0 ) * m_YRot() ) );
                    }
                }
                else
                {
                ...
                if ( m_ZRot() > 0 )
                {
                    if ( std::abs( ( M_PI / 180.0 ) * m_ZRot() ) > atan( ( del_x + 2 * x_off ) / ( del_y - 2 * y_off ) ) )
                    {
                        del_y_minus += std::abs( ( del_x + 2 * x_off ) / sin( ( M_PI / 180.0 ) * m_ZRot() ) );
                    }
                    else
                    {
                        del_y_minus += std::abs( ( del_y - 2 * y_off ) / cos( ( M_PI / 180.0 ) * m_ZRot() ) );
                    }

                    if ( std::abs( ( M_PI / 180.0 ) * m_ZRot() ) > atan( ( del_x - 2 * x_off ) / ( del_y + 2 * y_off ) ) )
                    {
                        del_y_plus += std::abs( ( del_x - 2 * x_off ) / sin( ( M_PI / 180.0 ) * m_ZRot() ) );
                    }
                    else
                    {
                        del_y_plus += std::abs( ( del_y + 2 * y_off ) / cos( ( M_PI / 180.0 ) * m_ZRot() ) );
                    }
                }
                else
                {
            ...
            else if ( m_OrientationPlane() == vsp::XY_BODY || m_OrientationPlane() == vsp::XY_ABS )
            {
                ...
                if ( m_YRot() > 0 )
                {
                    if ( std::abs( ( M_PI / 180.0 ) * m_YRot() ) > atan( ( del_z + 2 * z_off ) / ( del_x - 2 * x_off ) ) )
                    {
                        del_x_minus += std::abs( ( del_z + 2 * z_off ) / sin( ( M_PI / 180.0 ) * m_YRot() ) );
                    }
                    else
                    {
                        del_x_minus += std::abs( ( del_x - 2 * x_off ) / cos( ( M_PI / 180.0 ) * m_YRot() ) );
                    }

                    if ( std::abs( ( M_PI / 180.0 ) * m_YRot() ) > atan( ( del_z - 2 * z_off ) / ( del_x + 2 * x_off ) ) )
                    {
                        del_x_plus += std::abs( ( del_z - 2 * z_off ) / sin( ( M_PI / 180.0 ) * m_YRot() ) );
                    }
                    else
                    {
                        del_x_plus += std::abs( ( del_x + 2 * x_off ) / cos( ( M_PI / 180.0 ) * m_YRot() ) );
                    }
                }
                else
                {
                ...
                if ( m_XRot() > 0 )
                {
                    if ( std::abs( ( M_PI / 180.0 ) * m_XRot() ) > atan( ( del_z + 2 * z_off ) / ( del_y - 2 * y_off ) ) )
                    {
                        del_y_plus += std::abs( ( del_z + 2 * z_off ) / sin( ( M_PI / 180.0 ) * m_XRot() ) );
                    }
                    else
                    {
                        del_y_plus += std::abs( ( del_y - 2 * y_off ) / cos( ( M_PI / 180.0 ) * m_XRot() ) );
                    }

                    if ( std::abs( ( M_PI / 180.0 ) * m_XRot() ) > atan( ( del_z - 2 * z_off ) / ( del_y + 2 * y_off ) ) )
                    {
                        del_y_minus += std::abs( ( del_z - 2 * z_off ) / sin( ( M_PI / 180.0 ) * m_XRot() ) );
                    }
                    else
                    {
                        del_y_minus += std::abs( ( del_y + 2 * y_off ) / cos( ( M_PI / 180.0 ) * m_XRot() ) );
                    }
                }
                else
                {
            ...
            else if ( m_OrientationPlane() == vsp::XZ_BODY || m_OrientationPlane() == vsp::XZ_ABS )
            {
                ...
                if ( m_ZRot() > 0 )
                {
                    if ( std::abs( ( M_PI / 180.0 ) * m_ZRot() ) > atan( ( del_y + 2 * y_off ) / ( del_x - 2 * x_off ) ) )
                    {
                        del_x_plus += std::abs( ( del_y + 2 * y_off ) / sin( ( M_PI / 180.0 ) * m_ZRot() ) );
                    }
                    else
                    {
                        del_x_plus += std::abs( ( del_x - 2 * x_off ) / cos( ( M_PI / 180.0 ) * m_ZRot() ) );
                    }

                    if ( std::abs( ( M_PI / 180.0 ) * m_ZRot() ) > atan( ( del_y - 2 * y_off ) / ( del_x + 2 * x_off ) ) )
                    {
                        del_x_minus += std::abs( ( del_y - 2 * y_off ) / sin( ( M_PI / 180.0 ) * m_ZRot() ) );
                    }
                    else
                    {
                        del_x_minus += std::abs( ( del_x + 2 * x_off ) / cos( ( M_PI / 180.0 ) * m_ZRot() ) );
                    }
                }
                else 
                {
                ...
                if ( m_XRot() > 0 )
                {
                    if ( std::abs( ( M_PI / 180.0 ) * m_XRot() ) > atan( ( del_y + 2 * y_off ) / ( del_z - 2 * z_off ) ) )
                    {
                        del_z_plus += std::abs( ( del_y + 2 * y_off ) / sin( ( M_PI / 180.0 ) * m_XRot() ) );
                    }
                    else
                    {
                        del_z_plus += std::abs( ( del_z - 2 * z_off ) / cos( ( M_PI / 180.0 ) * m_XRot() ) );
                    }

                    if ( std::abs( ( M_PI / 180.0 ) * m_XRot() ) > atan( ( del_y - 2 * y_off ) / ( del_z + 2 * z_off ) ) )
                    {
                        del_z_minus += std::abs( ( del_y - 2 * y_off ) / sin( ( M_PI / 180.0 ) * m_XRot() ) );
                    }
                    else
                    {
                        del_z_minus += std::abs( ( del_z + 2 * z_off ) / cos( ( M_PI / 180.0 ) * m_XRot() ) );
                    }
                }
                else
                {
        ...
        rot_mat_x.rotate( ( M_PI / 180.0 ) * m_XRot(), x_axis );
        slice_surf.Transform( rot_mat_x );

        rot_mat_y.loadIdentity();
        rot_mat_y.rotate( ( M_PI / 180.0 ) * m_YRot(), y_axis );
        slice_surf.Transform( rot_mat_y );

        rot_mat_z.loadIdentity();
        rot_mat_z.rotate( ( M_PI / 180.0 ) * m_ZRot(), z_axis );
...
void FeaSpar::ComputePlanarSurf()
{
    ...
    if ( veh )
    {
        ...
        double alpha_0 = ( M_PI / 2 ) - signed_angle( inner_edge_vec, center_to_outer_edge, normal_vec ); // Initial rotation
        double theta;
        ...
        if( m_UsePercentChord() )
        {
            ...
            m_Theta.Set( -1 * ( ( 180.0 / M_PI ) * ( ( M_PI / 2 ) - alpha_f - alpha_0 ) ) );
        ...
        {
            theta = ( M_PI / 180.0 ) * m_Theta(); // User defined angle converted to Rad
        ...
        if ( m_BndBoxTrimFlag() )
        {
            ...
            if ( temp_slice )
            {
                ...
                temp_slice->m_ZRot.Set( ( 180.0 / M_PI ) * ( theta + alpha_0 ) );
        ...
        else
        {
            ...
            double beta_le = -1 * M_PI + signed_angle( center_to_inner_edge, lead_edge_vec, normal_vec ); // Angle between spar and leading edge
...
PolySparPoint::PolySparPoint()
{
    m_SpanMode.Init( "SpanMode", "FeaPolySparPoint", this, vsp::POLY_SPAR_POINT_ETA, vsp::POLY_SPAR_POINT_U01, vsp::NUM_POLY_SPAR_POINT_TYPES - 1 );

    m_Eta.Init( "Eta", "FeaPolySparPoint", this, 0.5, 0, 1.0 );
    m_Eta.SetDescript( "Rotation of Spar About Axis Normal to Wing Chord Line " );

    m_U01.Init( "U01", "FeaPolySparPoint", this, 0.5, 0, 1.0 );
    m_U01.SetDescript( "Rotation of Spar About Axis Normal to Wing Chord Line " );

    m_U0N.Init( "U0N", "FeaPolySparPoint", this, 0.2, 0, 1.0e12 );
    m_U0N.SetDescript( "Rotation of Spar About Axis Normal to Wing Chord Line " );

    m_XoC.Init( "XoC", "FeaPolySparPoint", this, 0.25, 0, 1.0 );
    m_XoC.SetDescript( "Rotation of Spar About Axis Normal to Wing Chord Line " );
}

void PolySparPoint::Update( WingGeom *wing )
{
    if ( wing )
    {
        double umax = wing->GetUMax(  0 );

        if ( m_SpanMode() == vsp::POLY_SPAR_POINT_ETA )
        {
            m_U0N = wing->EtatoU( m_Eta() );
            m_U01 = m_U0N() / umax;
        }
        else if ( m_SpanMode() == vsp::POLY_SPAR_POINT_U0N )
        {
            m_Eta = wing->UtoEta( m_U0N() );
            m_U01 = m_U0N() / umax;
        }
        else // m_SpanMode() == vsp::POLY_SPAR_POINT_U01
        {
            m_U0N = m_U01() * umax;
            m_Eta = wing->UtoEta( m_U0N() );
        }
...
//==== Parm Changed ====//
void PolySparPoint::ParmChanged( Parm* parm_ptr, int type )
{
    if ( type == Parm::SET )
    {
...
{
    PolySparPoint *pstart = AddPt();
    PolySparPoint *pend = AddPt();

    pstart->m_Eta.Set( 0.0 );
    pstart->m_XoC.Set( 0.25 );
    pstart->SetName( "Start" );
...
FeaPolySpar::~FeaPolySpar()
{
    for ( int i = 0; i < m_SparPointVec.size(); i++ )
    {
        delete m_SparPointVec[i];
    }
    m_SparPointVec.clear();
}

xmlNodePtr FeaPolySpar::EncodeXml( xmlNodePtr & node )
{
...
xmlNodePtr FeaPolySpar::DecodeXml( xmlNodePtr & node )
{
    DelAllPt();

    xmlNodePtr ps_node = FeaSlice::DecodeXml( node );

    xmlNodePtr sparpoints_node = XmlUtil::GetNode( ps_node, "SparPoints", 0 );

    if ( sparpoints_node )
    {
        int num = XmlUtil::GetNumNames( sparpoints_node, "SparPoint" );
        for ( int i = 0 ; i < num ; i++ )
        {
            xmlNodePtr spt_node = XmlUtil::GetNode( sparpoints_node, "SparPoint", i );
            if ( spt_node )
            {
                PolySparPoint* spt = AddPt();
                spt->DecodeXml( spt_node );
            }
        }
    }
...
void FeaPolySpar::ChangeID( const string &id )
{
    ...
    for ( int i = 0; i < m_SparPointVec.size(); i++ )
    {
        m_SparPointVec[i]->SetParentContainer( id );
    }
}
    ...
    if ( veh )
    {
        ...
        if ( wing )
        {
            ...
            for ( int i = 0; i < npt; i++ )
            {
                ...
                vec3d te = orig_surf.CompPnt01(  u, 0.0 );
                vec3d le = orig_surf.CompPnt01(  u, 0.5 );
                vec3d chrd = te - le;

                vec3d x = le + xoc * chrd;

                vec3d top = orig_surf.CompPnt01( u, 0.25 );
                vec3d bot = orig_surf.CompPnt01( u, 0.75 );
                vec3d z = top - bot;

                vec3d n = cross( chrd, z );
                vec3d dir = cross( n, chrd );
                dir.normalize();

                intersectRayAABB_inside( wing_bbox.GetMin().data(), wing_bbox.GetMax().data(), x.data(), dir.data(), upper_pts[i].data() );

                dir = -dir;
                intersectRayAABB_inside( wing_bbox.GetMin().data(), wing_bbox.GetMax().data(), x.data(), dir.data(), lower_pts[i].data() );

            }

            // Make Poly Planar Surface
            m_MainFeaPartSurfVec[0].MakePolyPlaneSurf( upper_pts, lower_pts );

            m_MainFeaPartSurfVec[0].BuildSimpleFeatureLines();
...
PolySparPoint* FeaPolySpar::AddPt()
{
    PolySparPoint *spt = new PolySparPoint();
    spt->SetParentContainer( m_ID );

    m_SparPointVec.push_back( spt );
    return spt;
}
...
PolySparPoint* FeaPolySpar::InsertPt( int index )
{
    if ( ValidPtIndex( index ) )
    {
        ...
        m_SparPointVec.insert( m_SparPointVec.begin() + index, rpt );
        return rpt;
    }
    return nullptr;
}
    ...
    if ( veh )
    {
        ...
        if ( current_wing )
        {
            ...
            if ( m_PerpendicularEdgeType() == vsp::TE_NORMAL )
            {
                ...
                alpha = ( M_PI / 2 ) - signed_angle( chord_dir_vec, trail_edge_dir_vec, m_WingNorm );
            }
            else if ( m_PerpendicularEdgeType() == vsp::LE_NORMAL )
            {
                ...
                alpha = ( M_PI / 2 ) - signed_angle( chord_dir_vec, lead_edge_dir_vec, m_WingNorm );
            }
            else if ( m_PerpendicularEdgeType() == vsp::SPAR_NORMAL )
            {
                ...
                if ( part && part->GetFeaPartSurfVec().size() > 0 )
                {
                    ...
                    alpha = ( M_PI / 2 ) - signed_angle( chord_dir_vec, spar_dir_vec, m_WingNorm );
                }
            }

            m_TotRot = alpha + m_Theta() * ( M_PI / 180.0 );
        }
    }
    ...
    if ( veh )
    {
        ...
        if ( m_BndBoxTrimFlag() )
        {
            ...
            if ( temp_slice )
            {
                ...
                temp_slice->m_ZRot.Set( ( 180.0 / M_PI ) * m_TotRot );
        ...
        else
        {
            ...
            double max_angle_inner_le = -M_PI + signed_angle( center_to_le_min_vec, center_to_lead_edge, m_WingNorm );
            double max_angle_inner_te = signed_angle( center_to_te_min_vec, center_to_trail_edge, m_WingNorm );
            double max_angle_outer_le = M_PI - signed_angle( center_to_lead_edge, center_to_le_max_vec, m_WingNorm );
            ...
            double phi_te = M_PI - ( m_TotRot + sweep_te ); // Total angle for trailing edge side of rib
            double phi_le = M_PI - ( m_TotRot + sweep_le );// Total angle for leading edge side of rib
